...
//
//
// This code is REALLY REALLY OLD.
// I really do not recommend using it as various basic features
// were never actually implemented (noteably moving)
//
//
...
public:
	...
	F1_CComponent (int x, int y, int w, int h)
	...
	F1_CComponent (F1_IComponent *parent, int x, int y)
	    : posX (x), posY (y), parent (parent)
	{
		posW = posH = 0;
		thisHandle  = ++lastHandle;
	}

	F1_CComponent (F1_IComponent *parent, int x, int y, int w, int h)
	    : posX (x), posY (y), posW (w), posH (h), parent (parent)
	{
		thisHandle = ++lastHandle;
	}
...
public:
	F1_CWindow (int x, int y)
	    : F1_CComponent (x, y), isMoving (false)
	{
		bounds    = {x, y, 0, 0};
		oldBounds = {0, 0};
	}

	F1_CWindow (int x, int y, int w, int h)
	    : F1_CComponent (x, y, w, h), isMoving (false)
	{
		bounds    = {x, y, w, h};
		oldBounds = {0, 0, 0, 0};
	}

	F1_CWindow (int x, int y, int w, int h, int bX, int bY, int bW, int bH)
	    : F1_CComponent (x, y, w, h), isMoving (false)
	{
		bounds    = {bX, bY, bW, bH};
		oldBounds = {0, 0, 0, 0};
