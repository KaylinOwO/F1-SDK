...
bool F1_CWindow::handleMouseInput( int x, int y, mouseButton m )
{
    ...
    else
    {
        ...
        F1_Rect newbounds = getBounds();

        if( CUtil::isPointWithinRange( {x, y}, {newbounds.x, newbounds.y}, {newbounds.w, newbounds.h} ) )
        {
...
void F1_CWindow::render()
{
    ...
    gDrawManager.OutlineRect( xy.x, xy.y, wh.x, wh.y, gDrawManager.dwGetTeamColor( GetBaseEntity( me )->GetTeam() ) );
